                    ...
                    @if ($branches->isEmpty())
                        ...
                        @if ($fallbackAddress)
                            <div class="bg-red-50 border border-red-100 rounded-xl px-4 py-3 flex items-start gap-3">
                                <div class="w-8 h-8 rounded-full bg-brand-gradient flex items-center justify-center shrink-0 mt-0.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-semibold text-gray-800 text-sm">DandeeJuice</p>
                                    <p class="text-gray-500 text-xs mt-0.5 leading-relaxed">{{ $fallbackAddress }}</p>
                                </div>
                            </div>
                        @else
                        ...
                        @endif
                    @else
                        <label class="block text-sm font-medium text-gray-700 mb-2">Select a branch</label>
                        ...
                        @if ($selectedBranchId)
                            @php $selectedBranch = $branches->firstWhere('id', (int) $selectedBranchId); @endphp
                            @if ($selectedBranch)
                                <div class="mt-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3 flex items-start gap-3">
                                    <div class="w-8 h-8 rounded-full bg-brand-gradient flex items-center justify-center shrink-0 mt-0.5">
                                        <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349m-16.5 11.65V9.35m0 0a3.001 3.001 0 003.75-.615A2.993 2.993 0 009.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 002.25 1.016c.896 0 1.7-.393 2.25-1.016a3.001 3.001 0 003.75.614m-16.5 0a3.004 3.004 0 01-.621-4.72L4.318 3.44A1.5 1.5 0 015.378 3h13.243a1.5 1.5 0 011.06.44l1.19 1.189a3 3 0 01-.621 4.72m-13.5 8.65h3.75a.75.75 0 00.75-.75V13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75v3.75c0 .415.336.75.75.75z"/>
                                        </svg>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="font-semibold text-gray-800 text-sm truncate">{{ $selectedBranch->name }}</p>
                                        @if ($selectedBranch->address)
                                            <p class="text-xs text-gray-500 mt-0.5 leading-relaxed">{{ $selectedBranch->address }}</p>
                                        @endif
                                        @if ($selectedBranch->phone)
                                            <p class="text-xs text-red-600 font-medium mt-1">{{ $selectedBranch->phone }}</p>
                                        @endif
                                    </div>
                                    <svg class="w-5 h-5 text-red-700 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                </div>
                            @endif
